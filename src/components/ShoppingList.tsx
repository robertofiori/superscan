...
export default function ShoppingList({ items, onUpdateQuantity, onStartScan }: ShoppingListProps) {
  ...
  return (
    <div className="w-full max-w-md flex flex-col h-full gap-6 animate-in fade-in">
      ...
      {items.length === 0 ? (
        <div className="bg-surface-light dark:bg-surface-dark rounded-[32px] p-10 text-center shadow-sm border border-slate-100 dark:border-slate-800 flex-1 flex flex-col items-center justify-center">
          ...
          <p className="text-slate-500 mb-8 max-w-[200px] mx-auto text-sm">Comienza a escanear productos para armar tu lista de compras económica.</p>
          <button 
            onClick={onStartScan}
        ...
        <>
          <div className="flex-1 space-y-4 overflow-y-auto min-h-[300px] pb-4 px-1">
            {items.map((item, idx) => (
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <h4 className="font-bold text-slate-800 dark:text-slate-100 text-sm leading-tight line-clamp-2 mb-1">
                      {item.product.product_name || 'Producto detectado'}
                    </h4>
                    <p className="text-xs text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md w-fit">
                      {item.price.supermarket}
                    </p>
                  </div>
                  <span className="font-black text-primary">${item.price.price.toLocaleString('es-AR')}</span>
                </div>

                <div className="shrink-0 flex flex-col items-center justify-between border-l border-slate-100 dark:border-slate-800 pl-3">
                  <button onClick={() => onUpdateQuantity(idx, 1)} className="w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-500 hover:text-black dark:hover:text-white transition-colors">
                    <Plus size={16} />
                  </button>
