  ...
  return (
    <div className="w-full relative" ref={dropdownRef}>
      <form onSubmit={handleSubmit} className="relative">
        ...
        {query && (
          <button 
            type="button" 
            onClick={() => setQuery('')}
            className="absolute inset-y-0 right-14 px-2 text-slate-400 hover:text-slate-600"
          >
            <X size={18} />
          </button>
        )}
        <button 
          type="submit"
          disabled={!query.trim()}
          className="absolute inset-y-2 right-2 bg-primary-green hover:bg-green-600 text-white font-bold px-4 rounded-xl transition-all shadow-md active:scale-95 disabled:opacity-50"
        >
          Ir
        </button>
      </form>
      ...
      {loading && query.length > 2 && !showDropdown && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl p-4 shadow-xl border border-slate-100 flex items-center justify-center gap-2">
          <Loader2 size={16} className="animate-spin text-primary-green" />
          <span className="text-xs font-bold text-slate-400">Buscando sugerencias...</span>
        </div>
      )}
    </div>
  );
